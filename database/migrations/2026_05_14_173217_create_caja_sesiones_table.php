...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('caja_sesiones', function (Blueprint $table) {
            ...
            $table->enum('estado', ['abierta', 'cerrada', 'auto_cerrada'])->default('abierta');
            $table->string('motivo_cierre', 60)->nullable();
            ...
            $table->index(['id_negocio', 'estado'], 'caja_sesion_negocio_estado_idx');
            $table->index('abierta_at', 'caja_sesion_abierta_at_idx');
            $table->index(['id_caja', 'estado'], 'caja_sesion_activa_lookup');

            $table->foreign('id_caja')
                ->references('id_caja')->on('cajas')->cascadeOnDelete();
