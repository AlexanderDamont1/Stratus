...
<div
    x-data="{
        ...
        deleteVoltaje: '',
        deleteAction: '',
        openDelete(id, modelo, voltaje, action) {
            this.deleteId = id;
            this.deleteModelo = modelo;
            this.deleteVoltaje = voltaje;
...
<div class="flex justify-between items-center">
    <div>
        ...
        <p class="text-xs text-gray-400 mt-0.5">
            Relaciona modelos con voltajes disponibles
        </p>
    </div>
    <button
...
<div class="grid grid-cols-3 gap-4">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow px-5 py-4">
        <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Total relaciones</p>
        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $relaciones->total() }}</p>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow px-5 py-4">
        <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Modelos activos</p>
        <p class="text-2xl font-semibold text-blue-600 dark:text-blue-400">{{ $modelos->count() }}</p>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow px-5 py-4">
        <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Voltajes disponibles</p>
        <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $voltajes->count() }}</p>
    </div>
</div>
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
    <div class="px-6 py-4 border-b dark:border-gray-700 flex items-center justify-between">
        ...
        <span class="text-xs text-gray-400">{{ $relaciones->total() }} total</span>
    </div>
        ...
        <table class="min-w-full text-sm border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900">
                @forelse($relaciones as $rel)
                    @php
                        $deleteRoute = route('gestor.vehiculos.modelo-voltaje.destroy', $rel);
                    @endphp
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <div class="h-6 w-6 rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center">
                                    <svg class="h-3 w-3 text-yellow-600 dark:text-yellow-400" 
                                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                        ...
                        <td colspan="4" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                            <div class="flex flex-col items-center justify-center">
                                <svg class="h-12 w-12 text-gray-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                          d="M20 12H4M12 4v16M4.5 4.5l15 15m-15 0l15-15"/>
                                </svg>
                                <p class="text-sm">No hay relaciones registradas aún</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <!-- Vista en móvil: tabla responsive -->
                            ...
                            <td class="px-3 py-3">
                                <div class="flex items-center">
                                    ...
                                    <div class="text-xs font-medium text-gray-900 dark:text-white">
                                        {{ Str::limit($rel->modelo->nombre_modelo ?? '—', 15) }}
                                    </div>
                                </div>
                            </td>
                            <td class="px-3 py-3">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-6 w-6 rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center mr-2">
                        ...
                        <tr>
                            <td colspan="3" class="px-3 py-8 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-xs">No hay relaciones</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
>
    ...
    >
            ...
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-lg bg-blue-600 dark:bg-blue-500 flex items-center justify-center shrink-0">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                              d="M13 10V3L4 14h7v7l9-11h-7z M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                    </svg>
                </div>
                <h3 class="text-base font-semibold text-gray-900 dark:text-white">Nueva relación</h3>
            </div>
            <button
        ...
        <form method="POST" action="{{ route('gestor.vehiculos.modelo-voltaje.store') }}">
            @csrf
            ...
            <div class="mb-5">
                ...
                >
                    ...
                    @foreach($voltajes as $voltaje)
                        <option value="{{ $voltaje->id_voltaje }}" {{ old('id_voltaje') == $voltaje->id_voltaje ? 'selected' : '' }}>
                            {{ $voltaje->voltaje }}
                        </option>
                    @endforeach
                </select>
...
>
    ...
    >
        <div class="flex items-start gap-4 mb-5">
            ...
            <div>
                <h3 class="text-base font-semibold text-gray-900 dark:text-white">Eliminar relación</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Vas a eliminar la relación entre 
                    <span class="font-semibold text-gray-700 dark:text-gray-300" x-text="deleteModelo"></span>
                    y 
                    <span class="font-semibold text-gray-700 dark:text-gray-300" x-text="deleteVoltaje"></span>.
