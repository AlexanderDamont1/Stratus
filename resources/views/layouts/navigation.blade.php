...
<div x-data="{ open: false }" class="flex overflow-hidden bg-gray-50 dark:bg-gray-950"
    ...
    : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}"
            ...
            @php
            $catalogoActivo = request()->routeIs('admin.catalogo.*');
            @endphp
            ...
            {{-- Precios (productos de la sucursal) --}}
    ...
    : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}"
        ...
        <div class="border-t dark:border-gray-700 p-4">
            <div class="flex items-center gap-3 mb-3 min-w-0">
                <div
                    class="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button
                    class="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="flex-1 flex flex-col min-w-0 ">
        <header
            class="lg:hidden h-16 flex items-center px-4 bg-white dark:bg-gray-800 border-b dark:border-gray-700 shrink-0 sticky top-0 z-20">
    ...
    <div x-data="{ step: {{ (Auth::user()->welcome_pending ?? true) ? 1 : 2 }} }"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            <div x-show="step === 1" x-transition:enter="transition ease-out duration-300"
                ...
                <button @click="step = 2"
                    class="w-full bg-gray-900 dark:bg-white dark:text-gray-900 text-white py-3 rounded-xl font-bold hover:opacity-90 transition shadow-lg">
            ...
            <div x-show="step === 2" x-transition:enter="transition ease-out duration-300"
                ...
                <form method="POST" action="{{ route('admin.setup.completar') }}" class="space-y-6">
                    ...
                    <div class="max-h-[50vh] overflow-y-auto pr-2 space-y-6">
                        @for($i = 0; $i < $max; $i++)
                            <div
                            class="p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
                            <h4 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-4">Vendedor
                                {{ $i + 1 }}
                            </h4>
                            <div class="grid gap-4 sm:grid-cols-2">
                                <div class="sm:col-span-2">
                                    <input type="text" name="vendedores[{{ $i }}][nombre]" placeholder="Nombre Sucursal"
                                        class="w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:text-white focus:ring-gray-900 text-sm sm:text-base"
                                        required>
                                </div>
                                <input type="email" name="vendedores[{{ $i }}][correo]" placeholder="Email de acceso"
                                    class="w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:text-white focus:ring-gray-900 text-sm sm:text-base"
                                    required>
                                ...
                                <div class="relative" x-data="{ showPassword: false }">
                                    <input :type="showPassword ? 'text' : 'password'"
                                        name="vendedores[{{ $i }}][password]" placeholder="Contraseña"
            ...
            <div class="pt-4 border-t dark:border-gray-800 flex gap-3">
                <button type="submit"
                    class="flex-1 bg-gray-900 dark:bg-white dark:text-gray-900 text-white py-4 rounded-xl font-bold shadow-xl hover:scale-[1.01] transition-transform">
