{{-- resources/views/administrador/cajas/corte-pdf.blade.php --}}
...
.row { display: table; width: 100%; margin-bottom: 2pt; }
...
@php
    ...
    $sucursal  = $corte->sesion->caja->usuario ?? null;
@endphp

<div class="biz-name">
    {{ $negocio?->nombre ?? config('app.name', 'Stratus') }}
</div>
<div class="biz-sub">Corte de caja — Administrador</div>
...
@php
    ...
    $motivo     = $sesion->motivo_cierre ?? null;
@endphp
...
@if($motivo === 'admin')
<div class="admin-badge">⚠ Cierre forzado por administrador</div>
@endif

{{-- ── Datos ── --}}
<div class="meta">Sucursal: <span class="meta-val">{{ $sucursal?->nombre_usuario ?? '—' }}</span></div>
...
<div class="meta">Generado por: <span class="meta-val">{{ $corte->usuario?->nombre_usuario ?? '—' }}</span></div>
...
<hr class="div-solid">
<div class="footer">
    {{ $tipo === 'parcial' ? 'Corte parcial — sesión en curso' : 'Sesión finalizada' }}<br>
    {{ $negocio?->nombre ?? config('app.name') }} · {{ $fecha }}
</div>
